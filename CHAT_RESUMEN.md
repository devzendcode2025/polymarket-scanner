...
- **Acciones B (scanner.py, db.py, polymarket_client.py, analyst.py, run_scan.py):** 🐛 **HALLAZGO CRÍTICO** — Gamma normaliza los precios (Yes+No=1 exacto en los 800 mercados barridos): el detector de mispricings por precios NUNCA podía encontrar desvíos (no era eficiencia del mercado, era la fuente). Fix: mispricings del LIBRO REAL vía CLOB /book de ambos tokens con caché SQLite (tabla orderbooks, TTL 10 min) y barrido en lotes de 30 mercados/corrida con keep-alive (181ms/libro; ~11s extra por corrida) para no romper el cron de 1 min. Detector nuevo `detect_mispricing_libro`: desvío del mid (bid+ask)/2 de cada token + arbitraje directo (ask_yes+ask_no<1 → comprar ambos garantizado; bid_yes+bid_no>1 → vender ambos). Umbral vol $10k→$5k, cola 200→600. Analista: degrada a confianza baja si libro ilíquido (liq<$10k y vol<$20k) y menciona liquidez en la razón.
- **Hallazgo adicional:** los libros de Polymarket están cruzados por el mismo MM (mid_yes+mid_no=1 por construcción) → hoy 0 mispricings de libro; el detector es real y capturará arbitrajes cuando el mercado se mueva fuerte. Es señal genuina, no ruido.
- **Rendimiento:** scan con caché caliente 19.6s (antes 7.8s); la corrida post-vencimiento de caché (cada 10 min) tarda ~74s (33 ligas ESPN + confirmación de pares nuevos) — el flock del cron absorbe el solapamiento (tick siguiente sale inmediato).
- **Resultado:** py_compile OK, scan real OK (2 edges New Hampshire +42/+37.5 pts vía PredictIt; 163 pares SI confirmados acumulados), data.json regenerado, `make test` TODO OK, v0.8.2 commiteada, cron reanudado.
- **Pendientes:** validar con días de datos que los edges visibles suban (hoy solo NH supera 8 pts); Telegram; backtest; histórico acumulando; on-ramps; Azuro; F5 Perú.
